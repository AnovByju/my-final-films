...
const MOVIES = [
  ...
  { id:4, title:"Whiplash", year:2014, lb:4.3, genres:["Drama","Music"], lang:"English",
    ...
    desc:"A promising young drummer enrolls at a cut-throat music conservatory where his instructor will stop at nothing to realize his potential.",
...
// ── THEME ───────────────────────────────────────────────────────────────────
const T = {
  gold:"#f5c518", bg:"#0f0f13", s1:"#1a1a24", s2:"#1e1e2a",
  tm:"rgba(255,255,255,0.55)", td:"rgba(255,255,255,0.28)",
  green:"#00c864", red:"#e63232",
};

// ── LAYOUT CONSTANTS ────────────────────────────────────────────────────────
const PH=750, PB=10, HDR=52, NAV=76, CH=750-PB*2-HDR-NAV; // CH=602
...
export default function MyFinalFilms() {
  ...
  const [mode,    setMode]    = useState("feed"); // feed|elim|winner
  const [em,      setEm]      = useState([]);
  const [ei,      setEi]      = useState(0);
  const [winner,  setWinner]  = useState(null);
  const [dragX,   setDragX]   = useState(0);
  const [dragging,setDragging]= useState(false);
  const [swipeAnim,setSwipeAnim]=useState(null);  // 'L'|'R'
  const [pAngle,  setPAngle]  = useState(0);
  const [pSpin,   setPSpin]   = useState(false);
  const [pResult, setPResult] = useState(null);
  const [pCool,   setPCool]   = useState(null);  // timestamp ms
  ...
  const spin = ()=>{
    ...
    setTimeout(()=>{
      setPResult(movies[Math.floor(Math.random()*movies.length)]);
      setPSpin(false);setPCool(Date.now()+2*60*60*1000);
    },3000);
  };

  // ── Shared helpers ──
  const imgF = id=>({background:MOVIES.find(m=>m.id===id)?.bg||T.bg});
  ...
  const renderFeed = ()=>(
    ...
    >
            ...
            <div key={mv.id} style={{height:CH,position:'relative',overflow:'hidden',background:mv.bg}}>
              ...
              <div style={{position:'absolute',right:11,bottom:110,display:'flex',flexDirection:'column',gap:15,alignItems:'center'}}>
                ...
                <button onClick={()=>pick(mv)} style={{background:'none',border:'none',cursor:'pointer',display:'flex',flexDirection:'column',alignItems:'center',gap:3}}>
                  <div style={{width:44,height:44,borderRadius:22,background:sel?'rgba(255,90,90,0.22)':'rgba(0,0,0,0.42)',border:`1.5px solid ${sel?'#ff5a5a':'rgba(255,255,255,0.22)'}`,display:'flex',alignItems:'center',justifyContent:'center',transition:'all 0.25s'}}>
                ...
                <button onClick={()=>save(mv)} style={{background:'none',border:'none',cursor:'pointer',display:'flex',flexDirection:'column',alignItems:'center',gap:3}}>
                  <div style={{width:44,height:44,borderRadius:22,background:sv?'rgba(245,197,24,0.18)':'rgba(0,0,0,0.42)',border:`1.5px solid ${sv?T.gold:'rgba(255,255,255,0.22)'}`,display:'flex',alignItems:'center',justifyContent:'center',transition:'all 0.25s'}}>
      ...
      {selected.length===3&&mode==='feed'&&(
        <div onClick={()=>{setEm(selected);setEi(0);setMode('elim');}}
          style={{position:'absolute',bottom:14,left:12,right:12,background:T.gold,borderRadius:14,padding:'13px 15px',display:'flex',alignItems:'center',justifyContent:'space-between',cursor:'pointer',zIndex:10,boxShadow:'0 4px 24px rgba(245,197,24,0.45)'}}>
    ...
    return (
      <div style={{height:CH,background:'#070710',display:'flex',flexDirection:'column',overflow:'hidden'}}>
        ...
        <div style={{flex:1,position:'relative',padding:'4px 14px 8px',overflow:'hidden'}}>
          <div
            style={{position:'absolute',inset:'4px 14px 8px',borderRadius:20,overflow:'hidden',background:cem.bg,
              transform:`translateX(${tx}px) rotate(${rot}deg)`,
              transition:swipeAnim?'transform 0.38s ease':dragging?'none':'transform 0.08s ease',
  ...
  const renderWinner = ()=>!winner?null:(
    <div style={{height:CH,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:22,background:`radial-gradient(ellipse at 50% 30%,rgba(245,197,24,0.07)0%,transparent 65%)`}}>
      ...
      <button onClick={reset} style={{background:T.gold,color:'#1a1000',border:'none',borderRadius:26,padding:'12px 28px',fontWeight:900,fontSize:15,cursor:'pointer',letterSpacing:0.3}}>Find Another Film</button>
    </div>
  );
  ...
  const renderSaved = ()=>(
    <div style={{height:CH,overflow:'auto',padding:16}}>
      <h2 style={{color:'white',fontSize:17,fontWeight:900,margin:'0 0 14px',letterSpacing:'-0.3px'}}>Saved Films</h2>
      {saved.length===0?(
          ...
          {saved.map(m=>(
            <div key={m.id} style={{borderRadius:12,overflow:'hidden',position:'relative',border:'1px solid rgba(255,255,255,0.07)',background:m.bg}}>
              ...
              <div style={{position:'absolute',bottom:7,left:8,right:24}}>
                <p style={{color:'white',fontSize:11,fontWeight:700,margin:0,lineHeight:1.2}}>{m.title}</p>
  ...
  const renderPicker = ()=>{
    ...
    return (
      <div style={{height:CH,overflow:'auto',display:'flex',flexDirection:'column',alignItems:'center',padding:'14px 18px'}}>
        <h2 style={{color:'white',fontSize:16,fontWeight:900,margin:'0 0 3px',letterSpacing:'-0.3px'}}>Random Picker</h2>
        ...
        <div style={{position:'relative',width:210,height:210,marginBottom:16,flexShrink:0}}>
          <div style={{position:'absolute',top:-9,left:'50%',transform:'translateX(-50%)',zIndex:5,
            width:0,height:0,borderLeft:'8px solid transparent',borderRight:'8px solid transparent',borderTop:'16px solid white'}}/>
          <svg width="210" height="210" viewBox="0 0 210 210" style={{
            transform:`rotate(${pAngle}deg)`,
            transition:pSpin?'transform 3s cubic-bezier(0.17,0.67,0.12,1)':'none',
            borderRadius:'50%',display:'block',overflow:'hidden',
          }}>
            {segs.map((mv,i)=>{
              ...
              return (
                <g key={mv.id}>
                  ...
                  <text x={tx} y={ty} textAnchor="middle" dominantBaseline="middle" fill="white" fontSize="8" fontWeight="bold"
                    transform={`rotate(${(ma+Math.PI/2)*180/Math.PI},${tx},${ty})`}>
                    {mv.title.length>7?mv.title.slice(0,6)+'…':mv.title}
                  </text>
                </g>
              );
            })}
        ...
        {pResult&&(
          <div style={{width:'100%',background:T.s1,borderRadius:14,overflow:'hidden',border:`1.5px solid ${T.gold}`,display:'flex'}}>
            ...
            <div style={{padding:'11px 13px'}}>
              ...
              <h4 style={{color:'white',margin:'0 0 4px',fontSize:14,fontWeight:900,letterSpacing:'-0.3px'}}>{pResult.title}</h4>
        ...
        <p style={{color:T.td,fontSize:11,textAlign:'center',marginTop:14,lineHeight:1.6}}>
          After spinning, the picker locks for 2 hours.<br/>Helps you commit to a film!
        </p>
      </div>
    );
  };
    ...
    <div style={{position:'absolute',inset:0,background:'rgba(0,0,0,0.66)',zIndex:60,display:'flex',flexDirection:'column',justifyContent:'flex-end'}} onClick={()=>setShowF(false)}>
      <div style={{background:'#14141e',borderRadius:'22px 22px 0 0',padding:'14px 16px',maxHeight:'66%',overflowY:'auto'}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{display:'flex',flexWrap:'wrap',gap:6,marginBottom:15}}>
          {ALL_GENRES.map(g=>{const a=filters.g.includes(g);return(
            <button key={g} onClick={()=>{setFilters(f=>({...f,g:a?f.g.filter(x=>x!==g):[...f.g,g]}));setFi(0);}}
              style={{padding:'5px 12px',borderRadius:20,background:a?T.gold:'rgba(255,255,255,0.08)',color:a?'#1a1000':'white',border:'none',cursor:'pointer',fontSize:12,fontWeight:a?700:400,transition:'all 0.2s'}}>{g}
            </button>
          );})}
        </div>
        <p style={{color:T.tm,fontSize:11,letterSpacing:1.5,margin:'0 0 8px',fontWeight:600}}>LANGUAGE</p>
        <div style={{display:'flex',flexWrap:'wrap',gap:6,marginBottom:16}}>
          {ALL_LANGS.map(l=>{const a=filters.l.includes(l);return(
            <button key={l} onClick={()=>{setFilters(f=>({...f,l:a?f.l.filter(x=>x!==l):[...f.l,l]}));setFi(0);}}
              style={{padding:'5px 12px',borderRadius:20,background:a?T.gold:'rgba(255,255,255,0.08)',color:a?'#1a1000':'white',border:'none',cursor:'pointer',fontSize:12,fontWeight:a?700:400,transition:'all 0.2s'}}>{l}
            </button>
          );})}
        </div>
  ...
  const renderHeader = ()=>(
    <div style={{height:HDR,background:'rgba(9,9,14,0.98)',borderBottom:'1px solid rgba(255,255,255,0.06)',display:'flex',alignItems:'center',padding:'0 14px',justifyContent:'space-between',position:'relative',zIndex:30}}>
      ...
      {mode==='feed'&&tab==='home'?(
        <button onClick={()=>setShowF(true)} style={{width:33,height:33,borderRadius:17,
          background:filters.g.length+filters.l.length>0?'rgba(245,197,24,0.13)':'rgba(255,255,255,0.06)',
          border:`1px solid ${filters.g.length+filters.l.length>0?T.gold:'rgba(255,255,255,0.1)'}`,
    ...
    return (
      <div style={{position:'absolute',bottom:0,left:0,right:0,height:NAV,background:'rgba(8,8,12,0.97)',borderTop:'1px solid rgba(255,255,255,0.06)',display:'flex',alignItems:'center',zIndex:40}}>
        {tabs.map(({id,Icon,label})=>{
          ...
          return (
            <button key={id} onClick={()=>{if(mode!=='elim'&&mode!=='winner')setTab(id);}}
              style={{flex:1,background:'none',border:'none',cursor:'pointer',display:'flex',flexDirection:'column',alignItems:'center',gap:3,padding:'10px 0 18px'}}>
  ...
  // ══════════════════ ROOT ══════════════════════════════════════════════════
  return (
    <div style={{display:'flex',justifyContent:'center',padding:'16px 0',minHeight:780}}>
      <style>{`
